...
>*/
	...
	function	warmStart(item)
	{
		...
		smooth_position = item.worldPosition()
	...
	function	OnUpdate(item)
	{
		...
		local	dt = (keep_behind ? target.matrix().GetRow(2) * -1 : item.worldPosition() - target_position).Normalize()
		...
		item.setPosition(smooth_position)
