...
class	CameraHandler
{
	...
	//------------------------------------
	{
		...
		target_vel = target_vel + v_d.Scale(0.5 / 60.0)
		speed = RangeAdjust(target_vel.Len(), 0.0, max_sneak_speed, 0.0, sneak_radius)
		speed = Clamp(speed, 0.0, sneak_radius) + Mtr(40.0)
	...
	//------------------------------------
	function	_FollowPlayerPosition(player_pos = Vector(0,0,0), player_vel = Vector(0,0,0))
	//------------------------------------
	{
			local	v_d = Vector(0,0,0)
