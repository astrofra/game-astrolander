...
>*/
	...
	function	Emit()
	{
		if ((g_clock - emit_clock) > SecToTick(emit_freq))
		{
			...
			pos = pos + Vector(Rand(-1,1), Rand(-1,1), 0).Normalize(_size).Scale(0.45)
			...
			ItemSetPosition(new_part, pos + Vector(0,0,Mtr(Rand(0.5, 1.0))))
			ItemSetRotation(new_part, Vector(0,0,Rand(0,180)))
			ItemSetScale(new_part, Vector(_size * 1.0,_size * 1.0,1.0))
